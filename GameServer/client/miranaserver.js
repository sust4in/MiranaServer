...
api methods
    initializeMiranaServer(path, options)
    addInput(input)
...
*/
var MiranaServerIO = (function (socketio) {

    "use strict";

    if (!socketio) {
        throw new Error("MiranaServer.IO: Socket.IO not found. Please ensure socket.io.js is referenced before the MiranaServer.io.js file.");
    }
    ...
    var _io = socketio,
        ...
        initializeMiranaServer = function (path, options) {
            _options = options;
            if (path == null || path.length <= 0) {
                throw new Error('MiranaServer.IO client is missing the server path - please verify the path argument passed to MiranaServerIO.initializeMiranaServer.');
            }
            _socket = _io.connect(path + '/miranaserver.io');
        ...
        registerSocketEvents = function () {
            _socket.on('connect', function () {
                ...
                if (_options.logging) {
                    console.log('miranaserver.io:: socket connect');
                }
            ...
            _socket.on('disconnect', function () {
                if (_options.logging) {
                    console.log('miranaserver.io:: socket disconnect');
                }
            ...
            _socket.on('reconnect', function () {
                if (_options.logging) {
                    console.log('miranaserver.io:: socket reconnect');
                }
            ...
            _socket.on('p', function (data) {
                ...
                if (_options.logging) {
                    console.log('miranaserver.io:: socket ping delay ' + _stateController.pingDelay);
                }
                if (_options.onPing) {
            ...
            _socket.on('rp', function (id) {
                removePlayer(id);
                if (_options.logging) {
                    console.log('miranaserver.io:: socket removePlayer ' + id);
                }
            ...
            _socket.on('re', function (data) {
                ...
                if (_options.logging) {
                    console.log('miranaserver.io:: socket removeEntity ' + data.id);
                }
            ...
            _socket.on('e', function (data) {
                if (_options.logging) {
                    console.log('miranaserver.io:: socket event ' + data);
                }
                if (_options.onEvent) {
    ...
    return {
        initializeMiranaServer: initializeMiranaServer,
        addInput: addInput,
...
MiranaServerIO.initializeMiranaServer
